...
plot.trim.fsa <- function(query.file =  "F-Ribotyping_Reps.lite.15/F001/F001.1.fsa",
                          query.ladder = rox.ladder,
                          query.channel= 1,
                          query.channel.ladder = 4,
  ...
  cdiff<- MLPA::read.fsa(query.file)
  cdiff.l <- cdiff[, query.channel.ladder]

  cdiff1 <- cdiff[, query.channel]
  ...
  plot(str1.x, cdiff1,  type = "l", xlim = c(  bp.min,   bp.max), ylim = range(  cdiff) *1.05,
       xlab = "bp", ylab = "peak intensity", lwd = 2, col = attr(cdiff, "colors")[query.channel])


  #lines(str1.x, cdiff.l,  type = "l", col = attr(cdiff, "colors")[query.channel.ladder])

  points( str1.x[str.1$time],
          cdiff1[str.1$time],
          pch = 21,
          bg = attr(cdiff, "colors")[query.channel])
...
trim.fsa <- function(file_path,
  ...
  for(i in 1 : num.channels){
    ...
    assign( paste0("channel", i) , channel.i)
  ...
  res.bp <- cbind(res[[channel.query]], bp)
  res.bp <-  res.bp [ res.bp$bp > min(ladder) & res.bp$bp < max(ladder) & !is.na(res.bp$bp), ]

  t <- res.bp$time
  b <- res.bp$bp

  lm.1 <- lm(b ~ t)
  bp.lm <- predict(lm.1, newdata = data.frame(t = 1 : length(channel1)))

  bp.lm[bp.lm >= bp.min & bp.lm <=  bp.max]

  plot(  bp.lm , channel1, xlim = c(bp.min, bp.max), type = "n", ylim = range(cdiff) * 1.05,
         ylab = "", xlab = "bp")

  for(i in 1 :  num.channels){

    s <- get(paste0("channel", i))
  ...
  points(res.bp$bp,
         res.bp$height,
         pch = 21,
         bg = attr(cdiff, "colors")[channel.query])

  text(res.bp$bp,
       res.bp$height,
       labels = round(res.bp$bp),
       col = attr(cdiff, "colors")[channel.query],
       pos = 4)
