...
sum.fsa <- function(file_path,
  ...
  if(plot){
    par(mar = c(5, 7, 2, 2))
    plot(cdiff[, 1], type = "n",
         ylim = c(-750, max(cdiff) * 1.075),
         axes = F, 
         ylab = "Fluorescence intensity\n\n",
         xlab = paste("file path =", file_path))
    axis(1, font = 3)
    axis(2, las = 2, font = 3)
  ...
  for(i in channels){
    ...
    if(revise.cutoff & i == channel.ladder){
      ...
      new.cutoff <-  median(  tallest ) - (sd(  tallest ) * sd.mult)
  ...
  names(res)[channels] <- names(attr(cdiff, "colors"))[channels]
  ...
  if(look4triplet & length(time) >= 15){
    ...
    if(length(adj) > 0){
      ...
      message("Warning: triplet in ladder not found")
    }
  }
  ######
  
  if(!all(peaks.needed %in% ladder)){
    
    missing.peaks <- paste("Warning: missing peaks at:", 
                           paste(peaks.needed[!peaks.needed %in% ladder], collapse = ", "),
                           "Bp")
    
    message(missing.peaks)
  ...
  if(plot){
    ...
    text(time[length(time)],
         ...
         ladder.id,
        # paste0(ladder.id, ", r^2", round(ladder.r2, 5)),
         col = attr(cdiff, "colors")[channel.ladder],
         pos = 4)
  }
  
  bp <- interpolate(stand.bp = ladder, 
                    stand.time = time,
                    sample = res[[channel.query]]$time)
  ...
  final.res <- list(
    result = res.bp,
    settings = list(file_path      = file_path,
                    ladder         = ladder,
                    cutoff         = cutoff,
                    ...
                    peaks.needed   = peaks.needed,
                    channel.ladder = channel.ladder,
                    channel.query  = channel.query),
    peaks_by_channel = res,
    metrics = list(max_ladder_intensity = max(res[[channel.ladder]]$height), 
                   max_query_intensity = max(res[[channel.query]]$height))
  )
...
pairwise.dist <- function(str.1 = database[[1]],  str.2, window = 2.5, method = "bc"){
  ...
  set.seed(42)
  suppressWarnings(hs <- vegan::rrarefy(hs, min(rowSums(hs))))
...
compare.frags <- function(query.file,
...
find.match <- function(file.query, 
  ...
  if(identical( bp.query, "no peaks found")){
    return("no match found")
  }
  
  bp.query$result <- filter.bp(bp.query$result)
  
  
  if(log.trans){
    bp.query$result$height <- log(bp.query$result$height)
    bp.query$result$area <- log(abs(bp.query$result$area))
  }
  
  if( nrow(  bp.query$result) > 0 ){
    
    db <- database$database
    res <- lapply(db,  function(db) pairwise.dist(str.1 = db,
                                                  str.2 = bp.query$result, 
                                                  method = meth))
    bp.query$matches <- sort(unlist(res))
...
make.database <- function(folder = "F-RibotypingFiles/F001/", ladder,  revise.cutoff= T,
                          cutoff = 100, sd.mult = 3, bp.min = 200, bp.max = 600){

  print("this may take a while please be patient")

  db.fsa <- list.files(folder, full.names = T, recursive = T, pattern = ".fsa")
  str.bp <- sum.fsa(db.fsa[1], ladder, cutoff, sd.mult = sd.mult)
  bp.query <- filter.bp(str.bp, bp.min, bp.max)

  summary.list <- lapply(db.fsa, sum.fsa, ladder, cutoff, sd.mult)
...
color_palette <- c( "#0072B2", "#D55E00", "#009E73", "#CC79A7", "#9370DB", "#F0E442",
                    "#56B4E9", "#FFAB30", "#6ACC64", "#C44E52", "#B47CC7", "#FFFF99",
                    "#00BFC4", "#9A82B8", "#8DB255", "#FF6B6B", "#FFD700", "#1A9850",
                    "#290333", "#909009", "#FDAE61", "#F46D43", "#D73027", "#A50026")

color.groups <- function (var = otu.sum$migration, cols = color_palette){
