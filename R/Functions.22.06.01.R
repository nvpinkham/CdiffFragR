...
sum.fsa <- function(file_path,
                    ...
                    sd = 3,
                    revise.cutoff = T,
                    ratio.cutoff = .25, # the highest query peak can not be lower than 25%  of the highest latter peak
                    only.up = T,
                    channel.ladder = 4,
                    channel.query = 1){
  ...
  ql.ratio <- max(res[[channel.query]]$height) / max(res[[channel.ladder]]$height)
  
  if(ql.ratio < ratio.cutoff){
    
    warning("Poor Query Signal")
    return("Poor Query Signal")
...
find.match <- function(file.query, ladder = rox.ladder,
  ...
  bp.query <- sum.fsa(file_path = file.query, ladder =  ladder, cutoff = cut.m, sd = sd.m) 
